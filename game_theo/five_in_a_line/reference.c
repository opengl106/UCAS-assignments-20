...
/////////////////////////////////////////////////////////////
int muda(int x, int y){
 if(x<0||x>14||y<0||y>14||(aRecordBoard[x][y]!=0)){
  return 1;
 }return 0;
}
int kinte(int x, int y, int PLAYER){
...
int bekaru(int x, int y, int PLAYER){
 if(x<0||x>14||y<0||y>14||aRecordBoard[x][y]!=PLAYER){
  return 0;
 }else{
  return 1;
 }
...
void displayBoard(void){
	...
	//第二步：将aDisplayBoardArray输出到屏幕上
        printf("Copyright (C) 高橋·Ｓ·毅航 2017.1.6\n");
        for(i=0;i<SIZE+1;i++){
...
struct point ret(){
 ...
 for(i=0;i<=14;i++){
  for(j=0;j<=14;j++){
   score[i][j]=10*autoscore(i, j, current_player)+9*autoscore(i, j, !(current_player-1)+1);
   if(kinte(i, j, current_player)){
    score[i][j]=0;
   }
  }
 }for(i=0;i<=14;i++){
...
int autoscore(int x, int y, int PLAYER){
 if(kinte(x, y, PLAYER)){
  ...
  if(PLAYER!=current_player){
   ...
   return (hi+120*hu+130*omi+12000*mi+105000*oyo+90000*his2+95000*his1+300000*yo+1000000*itsu);
  }
 }
}
